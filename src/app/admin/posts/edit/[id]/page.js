...
import { Loader2, Trash2, ArrowUp, ArrowDown, Image as ImageIcon, Type, Highlighter, Bold, Code, Save } from 'lucide-react';
...
export default function EditPostEditor() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const seriesRes = await fetch('/api/journal');
        const seriesData = await seriesRes.json();
        if (seriesData.success) setSeriesList(seriesData.data);

        const postRes = await fetch(`/api/posts/edit/${postId}`);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await fetch('/api/posts', {
        ...
        body: JSON.stringify({ _id: postId, title, slug, excerpt, coverImage, seriesId, sections })
      });
      const data = await res.json();
    ...
    <main className="min-h-screen bg-[#0a1120] text-white p-8 font-sans selection:bg-gold selection:text-navy">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-10 border-b border-white/10 pb-6">
          <h1 className="text-3xl font-bold uppercase tracking-widest text-gold">Edit Devlog</h1>
          <button onClick={handleUpdate} disabled={loading} className="flex items-center gap-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/50 px-8 py-3 rounded-full font-bold uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all">
            {loading ? <Loader2 className="animate-spin" /> : <><Save size={18} /> Update Log</>}
          </button>
        </div>
        ...
        <div className="bg-[#162033]/50 p-8 rounded-2xl border border-white/5 mb-10 space-y-6">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" value={title} onChange={handleTitleChange} className="w-full bg-[#0a1120] border border-white/10 rounded-lg p-4 text-white focus:border-gold outline-none" />
            </div>
          </div>
          
          <div>
            <label className="block text-xs uppercase text-slate mb-2 font-bold">Excerpt / Summary</label>
            <textarea value={excerpt} onChange={(e) => setExcerpt(e.target.value)} className="w-full bg-[#0a1120] border border-white/10 rounded-lg p-4 text-white focus:border-gold outline-none h-24" />
          </div>
            ...
            <div key={section.id} className="bg-[#162033]/30 p-6 rounded-2xl border border-white/5 relative group transition-all hover:border-gold/30">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2 opacity-30 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => moveSection(index, -1)} className="p-2 bg-[#0a1120] border border-white/10 rounded-lg hover:text-gold hover:border-gold/50 transition-all"><ArrowUp size={14} /></button>
                  <button onClick={() => moveSection(index, 1)} className="p-2 bg-[#0a1120] border border-white/10 rounded-lg hover:text-gold hover:border-gold/50 transition-all"><ArrowDown size={14} /></button>
                  <button onClick={() => removeSection(section.id)} className="p-2 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg hover:bg-red-500 hover:text-white transition-all"><Trash2 size={14} /></button>
                </div>
              </div>
